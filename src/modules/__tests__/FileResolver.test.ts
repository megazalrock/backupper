...
describe('FileResolver', () => {
  ...
  describe('ユニットテスト', () => {
    ...
    describe('isIncluded', () => {
      describe('ディレクトリパターン（末尾 /）', () => {
        test('src/ パターンで src/index.ts が true', () => {
          expect(isIncluded('src/index.ts', ['src/'])).toBe(true);
        });

        test('src/ パターンで src/lib/util.ts が true', () => {
          expect(isIncluded('src/lib/util.ts', ['src/'])).toBe(true);
        });

        test('src/ パターンで other/file.ts が false', () => {
          expect(isIncluded('other/file.ts', ['src/'])).toBe(false);
        });
      });

      describe('glob パターン', () => {
        test('**/*.ts パターンで src/index.ts が true', () => {
          expect(isIncluded('src/index.ts', ['**/*.ts'])).toBe(true);
        });

        test('*.json パターンで config.json が true', () => {
          expect(isIncluded('config.json', ['*.json'])).toBe(true);
        });

        test('**/*.ts パターンで src/index.js が false', () => {
          expect(isIncluded('src/index.js', ['**/*.ts'])).toBe(false);
        });
      });

      describe('単一ファイル（完全一致）', () => {
        test('config.json パターンで config.json が true', () => {
          expect(isIncluded('config.json', ['config.json'])).toBe(true);
        });

        test('config.json パターンで other.json が false', () => {
          expect(isIncluded('other.json', ['config.json'])).toBe(false);
        });

        test('config.json パターンで src/config.json が false（パスが異なる）', () => {
          expect(isIncluded('src/config.json', ['config.json'])).toBe(false);
        });
      });

      describe('複数パターン', () => {
        test('[\'src/\', \'*.json\'] で src/index.ts が true', () => {
          expect(isIncluded('src/index.ts', ['src/', '*.json'])).toBe(true);
        });

        test('[\'src/\', \'*.json\'] で config.json が true', () => {
          expect(isIncluded('config.json', ['src/', '*.json'])).toBe(true);
        });

        test('[\'src/\', \'*.json\'] で dist/bundle.js が false', () => {
          expect(isIncluded('dist/bundle.js', ['src/', '*.json'])).toBe(false);
        });
      });

      describe('エッジケース', () => {
        test('空の includes 配列で任意のパスが false', () => {
          expect(isIncluded('any/path.ts', [])).toBe(false);
        });
      });
    });
  });
  ...
  describe('インテグレーションテスト', () => {
    ...
    describe('filterByOnlyPattern', () => {
      ...
      test('拡張子でフィルタリングできる', () => {
        const result = filterByOnlyPattern(files, '*.ts');
        expect(result).toEqual(['src/index.ts', 'src/utils.ts']);
      });

      test('ディレクトリワイルドカードを含む拡張子でフィルタリングできる', () => {
        const result = filterByOnlyPattern(files, 'src/**/*.ts');
        expect(result).toEqual(['src/index.ts', 'src/utils.ts']);
      });

      test('特定のファイル名でフィルタリングできる', () => {
        const result = filterByOnlyPattern(files, 'README.md');
        expect(result).toEqual(['README.md']);
      });

      test('マッチしない場合は空配列を返す', () => {
        const result = filterByOnlyPattern(files, '*.css');
        expect(result).toEqual([]);
      });

      test('複雑なパターンでフィルタリングできる', () => {
        const result = filterByOnlyPattern(files, 'src/components/*.tsx');
        expect(result).toEqual(['src/components/Button.tsx', 'src/components/Header.tsx']);
      });
    });
  });
});
