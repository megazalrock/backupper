 ...
 * - それ以外 → 単一ファイルとして扱う
 */
export function resolveTargetFiles(cfg: ResolvedConfig): string[] {
  const files: string[] = [];
...
/**
 * パスが includes パターンに含まれるかどうかを判定する
 */
export function isIncluded(path: string, includes: string[]): boolean {
  for (const pattern of includes) {
    if (pattern.endsWith('/')) {
      // ディレクトリパターン: パスがこのディレクトリで始まるか確認
      const dirPattern = pattern.slice(0, -1); // 末尾の / を除去
      if (path.startsWith(dirPattern + '/') || path.startsWith(dirPattern)) {
        return true;
      }
 ...
 */
export function resolveRestoreFiles(cfg: ResolvedConfig): string[] {
  // outputDir内の全ファイルを取得
  ...
  for (const file of allFiles) {
    ...
    // includesパターンにマッチし、かつ excludesパターンに該当しない場合
    if (isIncluded(originalPath, cfg.includes) && !shouldExclude(originalPath, cfg.excludes)) {
...
export function findOrphanedFiles(
  ...
  for (const targetFile of targetFiles) {
    ...
    if (!sourceSet.has(targetFile)) {
      ...
      if (isIncluded(originalPath, config.includes) && !shouldExclude(originalPath, config.excludes)) {
...
/**
 * ファイル一覧を --only オプションの glob パターンでフィルタリングする
 */
export function filterByOnlyPattern(
  files: string[],
  pattern: string,
): string[] {
  const glob = new Bun.Glob(pattern);
  ...
  for (const file of files) {
    // パス全体とファイル名の両方でチェック（shouldExcludeと同様のロジック）
    if (glob.match(file) || glob.match(basename(file))) {
      matchedFiles.push(file);
    }
  }
